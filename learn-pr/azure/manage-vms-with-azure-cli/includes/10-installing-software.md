...
2. Aprire quindi una connessione `ssh` alla macchina come fatto in precedenza durante il test. Tenere presente che è necessario specificare il nome amministratore ("**aldis**").

3. Nella shell presentata, eseguire il comando seguente per installare il server Web `nginx`.

```azurecli
sudo apt-get -y update && sudo apt-get -y install nginx
```

4. In Azure Cloud Shell usare `curl` per leggere la pagina predefinita del server Web di Linux. In alternativa, è possibile aprire una nuova scheda del browser e navigare all'indirizzo IP pubblico.
...
5. Per aprire la porta 80 in Cloud Shell, digitare quanto segue:
