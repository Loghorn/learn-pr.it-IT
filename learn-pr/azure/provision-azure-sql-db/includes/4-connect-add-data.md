...
In questo caso, si utilizza il `az` per elencare i database e visualizzarne alcune informazioni sull'utilità il **logistica** database, tra cui la dimensione massima e lo stato.
...
1. Poiché si è interessati solo ai nomi dei database, eseguire il comando una seconda volta. Questa volta, inviare tramite pipe l'output a `jq` per visualizzare solo i campi del nome.
    ...
    ```
    **Logistics** è il database. Ad esempio SQL Server, **master** include metadati del server, ad esempio account di accesso e le impostazioni di configurazione di sistema.
...
1. Eseguire questo elenco di istruzioni T-SQL `SELECT` nella colonna `DriverID` di tutte le righe della tabella. Questa è l'operazione **read**.
