In questo caso, si creerà personalizzato le immagini del contenitore, effettuare il push in un registro contenitori pubblici e di eseguire i contenitori da queste immagini.

Le immagini del contenitore possono essere create manualmente o usando un Dockerfile per automatizzare il processo. Il metodo preferito consiste nell'usare un Dockerfile, ma questa unità illustrerà in entrambe le direzioni. Mettendo in pratica il processo manuale consente di comprendere meglio cosa accade quando si usa un Dockerfile per l'automazione.
...
In questo primo esempio, si avvia un'istanza di un contenitore che esegue Python, creare un'applicazione "Hello World" e quindi acquisizione il contenitore a una nuova immagine.

In primo luogo, eseguire un contenitore dall'immagine NGINX. Questo comando è leggermente diverso dai comandi eseguiti nell'unità precedente. Poiché si vuole stabilire una sessione terminal con il contenitore in esecuzione, vengono forniti gli argomenti `-t` e `-i`. Insieme, questi argomenti indicano a Docker di allocare una pseudo terminal che rimarrà in uno stato di esecuzione. In altre parole, gli argomenti `-t` e `-i` creano una sessione interattiva con il contenitore in esecuzione.
...
Dopo che il comando viene eseguito, la sessione terminal passerà allo pseudoterminale del contenitore. Ciò può essere osservato dal prompt di terminale, che dovrebbe essere stato modificato su un valore simile al seguente:

```output
root@d8ccada9c61e:/#
```

A questo punto, si lavorerà all'interno del contenitore. È possibile elencare, creare, eliminare i file, installare il software e apportare modifiche alla configurazione. Per questo semplice esempio, viene creato uno script basato su Python 'Hello World'. A tale scopo, eseguire il comando seguente:
...
Quando si è soddisfatti che lo script funzioni come previsto, chiudere il contenitore digitando `exit`:
...
Si noti che nessuno è in esecuzione. Quando è stato immesso `exit` nel contenitore in esecuzione, il processo Bash è stato completato e il contenitore è stato quindi arrestato. Si tratta del comportamento previsto.
...
Si noti che un contenitore con il nome *python-demo* ha lo stato *Exited*. Questo contenitore è l'istanza arrestata del contenitore in cui è appena stato chiuso.
...
Al termine dell'esecuzione del comando, si dovrebbe essere simile al seguente output:
...
Il contenitore avvierà e il messaggio "Hello World" di output. Il processo di Python viene quindi completato e il contenitore viene arrestato.
...
Nell'ultima sezione, si crea manualmente un'immagine del contenitore. Questo metodo funziona per la creazione dell'immagine occasionale o sperimentali, ma non sostenibile in un ambiente di produzione. La creazione di un'immagine del contenitore può essere automatizzata usando un *Dockerfile* e il comando `docker build`. Il *compilazione di docker* comando avvia un contenitore, esegue le istruzioni disponibili nella *Dockerfile*, quindi acquisisce i risultati in una nuova immagine contenitore.
...
Si verrà visualizzato output simile al seguente.
...
Mentre viene caricata l'immagine del contenitore Docker Hub, verrà visualizzato un output simile al seguente:
...
L'immagine del contenitore sono ora archiviata nell'Hub Docker ed è accessibile da qualsiasi computer connesso a Internet tramite `docker pull` o `docker run`.

È stato creato due immagini del contenitore. La prima è stata creata manualmente e la seconda è stata automatizzata usando un Dockerfile.