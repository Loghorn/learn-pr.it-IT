In questo esercizio si creerà un'applicazione AngularJS semplice ospitata in Node.js e si userà Express per il routing. Sul lato back-end, MongoDB fungerà da archivio dati. L'applicazione è un database di libri in cui sarà possibile elencare, aggiungere ed eliminare libri.

> [!Important]
> Si tratta di un'applicazione molto semplice il cui unico scopo è testare lo stack MEAN appena installato. Questa applicazione non è sufficientemente sicura o pronta per l'uso in un ambiente di produzione, pertanto si raccomanda di usarla nel modo appropriato.

## <a name="connect-to-the-vm"></a>Connettersi alla macchina virtuale

Se non ci si è ancora connessi alla macchina virtuale, eseguire il comando seguente. Sostituire i segnaposto `<vm-admin-username>` e `<vm-public-ip>` con il nome utente amministratore e l'indirizzo IP pubblico della macchina virtuale precedenti.
...
1. Installare **Express** per gestire il routing delle richieste HTTP, decidendo quale contenuto restituire a un utente dell'applicazione Web.
...
1. Creare il back-end del modello di dati per l'applicazione Web Books usando Mongoose.
    ...
    1. All'interno della cartella **app** nella cartella **Books** dell'applicazione creare un nuovo file JavaScript denominato **model.js** che contenga il modello di dati basato su Mongoose dell'applicazione.
    ...
    1. Incollare il codice seguente in questo nuovo file per creare lo schema dell'applicazione Books usando Mongoose.
        ...
        > [!Note]
        > Per salvare il file corrente in **nano** è necessario premere **CTRL**+**O** e per uscire da **nano** occorre premere **CTRL**+**X**
...
1. Creare le route Express per consentire all'applicazione di gestire le varie richieste HTTP.
    ...
    1. Incollare il codice seguente in questo nuovo file per stabilire le route mediante Express.
        ...
        Express può fornire le risposte HTTP direttamente nel codice di gestione delle route oppure può fornire il contenuto statico dei file. In questa applicazione Web di esempio vengono eseguite entrambe le operazioni, ossia rispondere con dati JSON alle richieste API di libri e rispondere con dati HTML direttamente dal file index.html.
