...
* Il numero di connessioni simultanee supportare superato il quale vengono restituiti errori.
...
Monitoraggio di Azure offre un singolo punto di gestione per i log di a livello di infrastruttura e il monitoraggio della maggior parte dei servizi di Azure. Il servizio raccoglie metriche, log attività, log di diagnostica e altro ancora. Monitoraggio di Azure offre una gamma di funzionalità, tra cui:

* Avvisi di Azure per inviare una notifica proattiva o intervenire in caso di violazioni di metriche o attività.
* Utilizzo di dashboard di Azure per racchiudere varie origini di monitoraggio in un'unica vista dell'applicazione.
...
Un criterio comune usato per determinare la disponibilità di un'applicazione Web è il criterio di monitoraggio endpoint di integrità. Il criterio viene usato per monitorare le applicazioni Web e i servizi di back-end associati per verificare che siano disponibili e funzionino correttamente. Il criterio viene implementato tramite la query a un URI specifico. L'endpoint controlla lo stato di molti componenti, inclusi i servizi back-end da cui dipende l'app, e non solamente la disponibilità del front-end stesso. Questa funzione agisce come un controllo di integrità a livello di servizio e restituisce un'indicazione dell'integrità complessiva del servizio.
...
Lamna Healthcare ha implementato un sistema di prenotazione basato su Web per i pazienti, basato su macchine virtuali e database SQL di Azure in due aree di Azure. Il team decide di usare l'agente di macchine virtuali e Log Analytics per monitorare le prestazioni delle macchine virtuali front-end sottostanti.
