Per vedere l'API Analisi del testo in azione, verranno effettuate alcune chiamate tramite lo strumento incorporato della console di test dell'API accessibile dalla documentazione di riferimento online. Prima di proseguire, è necessaria una chiave di accesso per eseguire tali chiamate. 
...
1. Accedere al portale di Azure all'indirizzo [https://portal.azure.com](https://portal.azure.com?azure-portal=true) con il proprio account Azure.

1.  Fare clic su **+ Crea una risorsa**.

1.  In Azure Marketplace selezionare **AI + Machine Learning** per visualizzare un elenco delle API disponibili. Fare clic su **Visualizza tutto** nell'angolo in alto a destra della pagina per visualizzare l'intero elenco delle API Servizi cognitivi. 

1. Trovare **Analisi del testo** nell'elenco di Servizi cognitivi e selezionarla. 
![Screenshot del riquadro AI + Machine Learning, con l'API Analisi del testo selezionata nell'elenco.](../media-draft/select-text-analytics.PNG)

1. Nella pagina per la creazione che si aprirà immettere i valori seguenti in ogni campo.
...
|Sottoscrizione     |  Sottoscrizione in uso       |   La sottoscrizione in cui viene creato il nuovo account dell'API Servizi cognitivi con l'**API Analisi del testo**.      |
|Località     |  Stati Uniti occidentali       |  Scegliere un'[area](https://azure.microsoft.com/regions/) nelle vicinanze.       |
...
|Gruppo di risorse     |  Creare un nuovo gruppo di risorse e chiamarlo [!INCLUDE [resource-group-note](./rg-name.md)].       |  Nome del nuovo gruppo di risorse in cui creare l'account dell'API Analisi del testo di Servizi cognitivi.       |


[!INCLUDE [resource-group-note](./rg-notice.md)]

Di seguito è riportato uno screenshot della pagina **Crea** con le opzioni completate.
...
6. Selezionare **Crea** nella parte inferiore della pagina per avviare il processo di creazione dell'account.  Attendere la notifica che la distribuzione è in corso. Si riceverà quindi una notifica che l'account è stato distribuito correttamente al gruppo di risorse.
...
Ora che si dispone dell'account di Servizi cognitivi, è il momento di trovare la chiave di accesso in modo da iniziare a chiamare l'API. 

7. Fare clic sul pulsante **Vai alla risorsa** all'interno della notifica di *distribuzione completata*. Questa azione apre l'Avvio rapido dell'account. 
...
> [!IMPORTANT]
> Conservare sempre le chiavi di accesso in un luogo sicuro e non condividerle con altri utenti. 

10. Tenere da parte questa chiave per il resto del modulo. Verrà usata a breve per effettuare chiamate API dalla console di test e nella parte restante del modulo.

Ora che si dispone della chiave, è possibile passare alla console di test e provare l'API.
...
- Aggiungere la chiave di accesso salvata in precedenza in questa lezione per ogni chiamata.

2. Dal menu a sinistra selezionare **Sentiment**. Questa selezione consente di aprire la documentazione per l'analisi del sentiment a destra. Come illustrato nella documentazione, si effettuerà una chiamata REST nel formato seguente:
...
![Screenshot del sito di riferimento dell'API Analisi del testo con la voce di menu Sentiment selezionata, seguita da Stati Uniti occidentali.](../media-draft/select-testing-console-region.png)
...
![Sezione numero quattro.](../media-draft/4-marker.PNG) 
![Sezione numero cinque.](../media-draft/5-marker.PNG) 
...
![Sezione numero cinque.](../media-draft/7-marker.PNG)  
