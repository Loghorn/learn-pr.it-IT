...
1. Creare una classe denominata `MainViewModel` nel progetto .NET Standard `ImHere`.
...
1. Aggiungere un costruttore alla classe e in questo costruttore inizializzare `SendLocationCommand` come nuovo Xamarin.Forms `Command`. È necessario aggiungere una direttiva utilizzo per lo spazio dei nomi `Xamarin.Forms`. Il costruttore per questo comando impiega `Action` per l'esecuzione quando il comando viene richiamato; quindi creare un metodo `async` denominato `SendLocation` e passare una funzione lambda che `await` questa chiamata al costruttore. Il corpo del metodo `SendLocation` verrà implementato nelle unità successive in questo modulo. È necessario aggiungere una direttiva utilizzo per lo spazio dei nomi `System.Threading.Tasks` perché venga restituito un `Task`.
...
1. Aprire il file `MainPage.xaml` dal progetto `ImHere`. La pagina verrà aperta nell'editor XAML.
    ...
    > [!NOTE]
    >  Il progetto `ImHere.UWP` contiene anche un file denominato `MainPage.xaml`. Assicurarsi di modificare tale file nella libreria .NET Standard.
...
1. Sovrascrivere `StackLayout` con il codice seguente:
    ...
    ```
    - Il controllo `Editor` verrà usato per aggiungere i numeri di telefono e `Label` riportato sopra descrive lo scopo di questo campo per l'utente. 
    - `StackLayout` distribuisce i controlli figlio in senso orizzontale o verticale nell'ordine in cui vengono aggiunti i controlli, pertanto aggiungendo prima `Label`, questo verrà inserito prima di `Editor`.
    - I controlli `Editor` sono controlli voce su più righe che consentono all'utente di immettere più numeri di telefono, uno per riga.
...
1. Aggiungere un `Button` sotto al controllo `Editor`. Questo pulsante verrà usato per inviare la posizione dell'utente.
...
1. Aggiungere un `Label` sotto al controllo `Button`. I messaggi di stato verranno visualizzati in questa etichetta.
...
1. Eseguire l'app per visualizzare la nuova interfaccia utente. Se, a questo punto, si intende convalidare l'associazione, è possibile farlo mediante l'aggiunta di punti di interruzione alle proprietà o al metodo `SendLocation`.

    > [!NOTE]
    > Durante la compilazione dell'app, verrà visualizzato un avviso relativo alla `SendLocation` mancanza di modificatori `await`. È possibile ignorare questo avviso in quanto verrà risolto non appena verrà aggiunto altro codice a questo metodo nell'unità successiva.
