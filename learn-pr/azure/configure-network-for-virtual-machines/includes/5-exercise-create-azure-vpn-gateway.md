Garantire la connessione di client o siti del proprio ambiente in Azure tramite tunnel crittografati tramite la rete Internet pubblica. In questo esercizio verrà creato un gateway VPN da punto a sito e ci si connetterà al gateway da un computer client. Per la sicurezza, verranno usate connessioni di autenticazione del certificato nativa di Azure.
...
1. Generare un certificato client da un certificato radice, quindi installare il certificato client in ogni computer client che si connetterà alla rete virtuale ai fini dell'autenticazione

1. Creare i file di configurazione del client VPN, che contengono le informazioni necessarie per la connessione del client alla rete virtuale.
...
## <a name="configure-a-vnet"></a>Configurare una rete virtuale
...
1. Creare la rete virtuale usando i valori della subnet e un server DNS statico. 
    > [!IMPORTANT]
    > Ignorare il messaggio di avviso, quindi attendere il completamento del comando.
...
> [!NOTE]
> Si noti che questa parte dell'esercizio può richiedere fino a 45 minuti, a seconda del valore dello SKU del gateway
...
1. Nella pagina **File da esportare** in **Nome file** immettere **C:\cert\P2SRootCert.cer**, quindi fare clic su Avanti.
