...
Oltre a ciò, più frequenti sono le interazioni con un'applicazione, più round trip sono necessari. Ogni round trip implica un costo in termini di latenza e ogni round trip contribuisce alla latenza complessiva. La figura seguente mostra come la latenza percepita dall'utente è una combinazione tra il numero di round trip necessari per soddisfare la richiesta.

![Un'illustrazione che mostra la latenza di rete tra le risorse inserito in località geografiche diverse nel cloud.](../media/3-networkLatency.png)
...
Può essere utile Gestione traffico di Microsoft Azure. Gestione traffico è un servizio di bilanciamento del carico basato su DNS che ti permette di distribuire il traffico all'interno e tra aree di Azure. Anziché chiedere all'utente di passare a un'istanza specifica del front-end web, gestione traffico può indirizzare agli utenti in base a un set di caratteristiche:
...
- **Prestazioni** -gestione traffico indirizza gli utenti per l'istanza più vicina front-end basato su latenza di rete.
...
Il team di distribuisce un'istanza di gestione traffico in modalità a prestazioni e aggiunta le due istanze front-end come profili di gestione traffico. Un utente finale, passare a un nome di dominio personalizzato (ad esempio, lamnahealthcare.com) che esegue il routing a gestione traffico. Gestione traffico restituisce quindi il nome DNS dell'Europa occidentale o orientale Australia front-end basato sulle migliori prestazioni di latenza di rete.
