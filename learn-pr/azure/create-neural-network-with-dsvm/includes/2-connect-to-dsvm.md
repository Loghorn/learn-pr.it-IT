...
1. Se non è già disponibile un client Xfce installato, scaricare il [client X2Go](https://wiki.x2go.org/doku.php/download:start) e installarlo prima di continuare con questo esercizio. X2Go è una soluzione di Xfce gratuita e open source che funziona su una vasta gamma di sistemi operativi, tra cui Windows e OS X. Le istruzioni in questa unità presuppongono si usa X2Go, ma è possibile usare qualsiasi client che supporta Xfce.
...
1. Immettere la password specificata nell'ultima unità e quindi scegliere il **OK** pulsante. Se viene richiesto se si considera attendibile la chiave host, rispondere **Yes** (Sì). Ignorare inoltre eventuali messaggi di errore che segnalano che non è stato possibile avviare il daemon SSH.
