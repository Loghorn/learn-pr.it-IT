...
Mentre i domini di aggiornamento rappresentano una sezione logica del data center, i domini di errore ne rappresentano le sezioni fisiche e garantiscono la diversità dei rack dei server in un set di disponibilità. I domini di errore sono allineati alla separazione fisica dell'hardware condiviso nel data center. Questo include l'alimentazione, il raffreddamento e l'hardware di rete che supporta i server fisici nei rack. Se l'hardware che supporta un rack di server diventa non disponibile, solo tale rack di server è interessato dall'interruzione. Se si includono le macchine virtuali in un set di disponibilità, tali macchine vengono distribuite automaticamente su più domini di errore. Pertanto il raro caso di un guasto hardware avrà effetto solo su una parte delle macchine virtuali.
...
* **Gestione traffico di Azure** offre il bilanciamento del carico DNS globale. È consigliabile prendere in considerazione Gestione traffico per rendere disponibile il bilanciamento del carico degli endpoint DNS all'interno delle aree di Azure o tra le aree stesse.
