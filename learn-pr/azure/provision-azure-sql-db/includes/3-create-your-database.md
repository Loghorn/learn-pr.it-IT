...
Nel corso del tempo se che si rende che la necessità di ulteriori calcolo potenza per soddisfare la richiesta, è possibile modificare opzioni relative alle prestazioni o persino passare tra vCore e DTU modelli di prestazioni.
...
1. Nell'angolo superiore sinistro del portale fare clic su **Crea una risorsa**. Selezionare **Database**, quindi selezionare **Database SQL**.
   ...
   ![Screenshot del portale di Azure con la creazione di un pannello della risorsa con la sezione database selezionati e la crea una risorsa, database e Database SQL di pulsanti evidenziati.](../media-draft/create-db.png)
...
1. Usare i valori seguenti per compilare il resto del modulo.
    ...
    | **Gruppo di risorse** |  Usare il gruppo esistente <rgn>[nome gruppo di risorse di tipo Sandbox]</rgn> |
...
Al termine del processo, fare clic su **Aggiungi al dashboard** per aggiungere il server di database al dashboard, in modo da potervi accedere rapidamente in un secondo momento.
   ...
   ![Screenshot del portale di Azure che mostra il menu di notifiche con il Pin al pulsante dashboard da un messaggio di conferma distribuzione recente evidenziato.](../media-draft/notifications-complete.png)
...
1. Fare clic su **Imposta firewall server**.
    ...
    ![Screenshot del portale di Azure che mostra un pannello di panoramica di database SQL con il pulsante firewall del server Set evidenziato.](../media-draft/set-server-firewall.png)

1. Fare clic su **Aggiungi IP client** e fare clic su **Salva**.

    ![Screenshot del portale di Azure con un database SQL pannello impostazioni del Firewall con il pulsante IP client Aggiungi evidenziato.](../media-draft/add-client-ip.png)
