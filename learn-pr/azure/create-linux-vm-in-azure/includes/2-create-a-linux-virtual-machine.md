A questo punto è disponibile un sito Web esistente in esecuzione in un server Ubuntu Linux locale. L'obiettivo è creare una VM di Azure usando l'immagine Ubuntu più recente ed eseguire la migrazione del sito nel cloud. In questa unità verranno presentate le opzioni per le quali sarà necessario prendere una decisione durante la creazione di una macchina virtuale in Azure.
...
Come per altri servizi di Azure, sarà necessario un **gruppo di risorse** per contenere la macchina virtuale (e, facoltativamente, raggruppare queste risorse per l'amministrazione). Quando si crea una nuova VM, è possibile usare un gruppo di risorse esistente o crearne uno nuovo.
...
Tutto ciò che può essere installato in un computer può essere incluso in un'immagine. È possibile creare una macchina virtuale da un'immagine preconfigurata esattamente per le attività necessarie, ad esempio l'hosting di un'app Web in un server Apache.

> [!TIP]
> È anche possibile creare e caricare immagini di disco personalizzate. Per altre informazioni, vedere la documentazione.
...
| Cosa si farà? | Dimensioni da considerare
|-------|------------------|
...
1. Il **disco del sistema operativo**. Questa è l'unità principale e ha una capacità massima di 2048 GB. Sarà denominata `/dev/sda` per impostazione predefinita.

1. Un **disco temporaneo**. Questo fornisce l'archiviazione temporanea per il sistema operativo o eventuali app. Nelle macchine virtuali Linux il disco è `/dev/sdb` e viene formattato e montato in `/mnt` dall'agente Linux di Azure. Le dimensioni vengono stabilite in base alla dimensione della macchina virtuale e viene usato per archiviare il file di scambio. 
...
- **Migliore sicurezza**: i dischi gestiti sono risorse gestite reali nel gruppo di risorse. Questo significa che possono usare controllo degli accessi in base al ruolo per limitare chi può utilizzare i dati dei dischi rigidi virtuali.
- **Supporto degli snapshot**: è possibile usare gli snapshot per creare una copia di sola lettura di un disco rigido virtuale. È necessario arrestare la macchina virtuale proprietaria, ma creazione dello snapshot richiede solo pochi secondi. Al termine, è possibile accendere la macchina virtuale e usare lo snapshot per creare una macchina virtuale duplicata per risolvere un problema di produzione o ripristinare la macchina virtuale al punto in cui è stato creato lo snapshot.
...
Le macchine virtuali comunicano con le risorse esterne tramite una rete virtuale (VNet). La rete virtuale rappresenta una rete privata in un'unica area, usata dalle risorse per le comunicazioni. Una rete virtuale è esattamente come le reti gestiti in locale. È possibile suddividerla tramite subnet per isolare le risorse, connetterla ad altre (incluse le reti locali) e applicare le regole del traffico per regolare le connessioni in ingresso e in uscita.
