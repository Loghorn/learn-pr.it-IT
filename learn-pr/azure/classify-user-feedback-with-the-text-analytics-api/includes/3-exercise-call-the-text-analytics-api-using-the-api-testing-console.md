...
1. Nella pagina **Crea** che si aprirà immettere i valori seguenti in ogni campo.
    ...
    |Località     |  *scegliere una regione dall'elenco a discesa*       |  Poiché si usa l'ambiente sandbox gratuito, selezionare una località nell'elenco a discesa presente **anche** nell'elenco delle aree sandbox seguente.  |
    ...
    ### <a name="sandbox-region-list"></a>Elenco delle aree sandbox
    [!include[](../../../includes/azure-sandbox-regions-first-mention-note-friendly.md)]

    > [!TIP]
    > Ricordare la località selezionata al momento della creazione dell'account Servizi cognitivi Analisi del testo. Servirà a breve per effettuare chiamate API. 
...
1. Passare all'URL seguente nel browser preferito. Sostituire `[location]` con la località selezionata al momento della creazione dell'account Servizi cognitivi Analisi del testo più indietro nell'unità. Ad esempio, se l'account è stato creato in *Stati Uniti orientali*, occorre sostituire `[location]` con `eastus` all'interno dell'URL.
...
1. Dal menu a sinistra selezionare **Valutazione**. Questa selezione consente di aprire la documentazione per l'analisi del sentiment a destra. Come illustrato nella documentazione, si effettuerà una chiamata REST nel formato seguente.  
    ...
    `https://[location].api.cognitive.microsoft.com/text/analytics/v2.0/sentiment`
     ...
     `[location]` viene sostituito con la località selezionata al momento della creazione dell'account Analisi del testo.  
...
1. Selezionare il pulsante **Open API testing console** (Apri console di test API) per aprire la console API in tempo reale e in modalità interattiva.

1. Incollare la chiave di accesso salvata in precedenza nel campo con l'etichetta **Ocp-Apim-Subscription-Key**. Si noti che la chiave viene scritta automaticamente nella finestra della richiesta HTTP come valore di intestazione.

1. Scorrere fino alla parte inferiore della pagina e fare clic su **Invia**.

    Ogni sezione della schermata verrà esaminata in modo più dettagliato.
    ...
    La richiesta completa, incluse le intestazioni e l'URL della richiesta, è visualizzata nella sezione successiva. In questo esempio si noterà che le richieste vengono indirizzate a un URL che inizia con `westus`. L'URL è diverso a seconda della località selezionata al momento della creazione dell'account Analisi del testo.
