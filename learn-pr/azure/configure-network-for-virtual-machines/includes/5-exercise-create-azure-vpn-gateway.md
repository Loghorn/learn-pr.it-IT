Garantire la connessione di client o siti del proprio ambiente di Azure tramite tunnel crittografati sulla rete Internet pubblica. In questa unità verrà creato un gateway VPN da punto a sito e ci si connetterà al gateway da un computer client. Per la sicurezza verranno usate connessioni di autenticazione del certificato native di Azure.
...
1. Generare un certificato client dal certificato radice, quindi installare il certificato client in ogni computer client che si connette alla rete virtuale ai fini dell'autenticazione.

1. Creare file di configurazione del client VPN con le informazioni necessarie per la connessione del client alla rete virtuale.
...
1. Per creare il gateway VPN eseguire il comando seguente e premere INVIO.
...
1. Nella pagina **Formato file di esportazione** assicurarsi che sia selezionata l'opzione **Codificato Base 64 X.509 (.CER)** e quindi fare clic su **Avanti**.

1. Nella pagina **File da esportare** in **Nome file** immettere **C:\cert\P2SRootCert.cer** e fare clic su Avanti.
...
1. Copiare l'URL restituito nell'output del comando e incollarlo nel browser. Il browser avvia il download di un file con estensione ZIP. Decomprimere il file e salvarlo nel percorso desiderato.
...
1. Fare doppio clic sul file VpnClientSetupxxxxx.exe, dove xxxxx varia a seconda dell'architettura in uso.
...
È stato configurato un gateway VPN per stabilire una connessione client crittografata a una rete virtuale in Azure. Questo approccio è ideale per i computer client e le connessioni da sito a sito con volumi ridotti.