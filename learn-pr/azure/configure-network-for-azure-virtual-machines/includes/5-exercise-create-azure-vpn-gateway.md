Garantire la connessione di client o siti del proprio ambiente in Azure tramite tunnel crittografati tramite la rete Internet pubblica. In questa unità, sarà creare un gateway VPN da punto a sito e quindi connettersi a tale gateway da un computer client. Per la sicurezza, verranno usate connessioni di autenticazione del certificato nativa di Azure.

Procedura:

1. Creare un gateway VPN RouteBased.

1. Caricare la chiave pubblica per un certificato radice per scopi di autenticazione.

1. Generare un certificato client dal certificato radice e quindi installare il certificato client in ogni computer client che si connetteranno alla rete virtuale per scopi di autenticazione.

1. Creare i file di configurazione, che contengono le informazioni necessarie per il client per connettersi alla rete virtuale di client VPN.

## <a name="before-you-begin"></a>Prima di iniziare
<!---TODO: These should be prerequisites in the first unit and on the index.yml--->

Per completare questo modulo, è necessario disporre di:

- Azure PowerShell installato

- Una cartella denominata **C:\cert**

Per installare Azure PowerShell:

1. Fare doppio clic sul pulsante Windows e fare clic su **PowerShell (Admin)**.
...
1. Immettere le variabili seguenti e premere INVIO dopo ognuna di esse.
...
1. Creare la rete virtuale usando i valori della subnet e un server DNS statico.

    > [!IMPORTANT]
    > Ignorare il messaggio di avviso e quindi attendere il completamento del comando.
...
- VpnType deve essere di tipo RouteBased

> [!NOTE]
> Si noti che questa parte dell'esercizio può richiedere fino a 45 minuti, a seconda del valore di GatewaySku.

1. Per creare il gateway VPN, immettere il comando seguente e premere INVIO.
...
1. Passare a Personale/Certificati. Fare clic su P2SRootCert, fare clic su **tutte le attività**, quindi selezionare **esportare**.

1. In Esportazione guidata certificati fare clic su **Avanti**.

1. Assicurarsi che **No, non esportare la chiave privata** sia selezionata e quindi fare clic su **successivo**.

1. Nel **formato File di esportazione** pagina, assicurarsi che **con codifica Base 64 X.509 (. Area a esecuzione vincolata)** sia selezionata e quindi fare clic su **successivo**.

1. Nel **File da esportare** nella pagina **nome File**, immettere **C:\cert\P2SRootCert.cer**e quindi fare clic su Avanti.
...
1. Eseguire il comando seguente per creare file di configurazione client VPN in. Formato ZIP.
...
1. Copiare l'URL restituito nell'output di questo comando e incollarlo nel browser. Il browser avvierà il download di un file ZIP. Decomprimere il file e salvarlo nel percorso desiderato.

1. Nella cartella di estrazione, passare alla cartella WindowsAmd64 (per i computer Windows a 64 bit) o la cartella WindowsZX86 (per computer a 32 bit).

1. Fare doppio clic sul file VpnClientSetupxxxxx.exe, a seconda dell'architettura in uso.

1. Nel **Windows protetti del PC** schermata, fare clic su **altre informazioni**e quindi fare clic su **eseguire comunque**.
...
1. Nel riquadro di destra, fare clic su **VNetData**, quindi fare clic su **Connect**.
...
> [!NOTE]
> Se questi passaggi non funzionano, si potrebbe essere necessario riavviare il computer.
...
È sufficiente impostare un gateway VPN, che consente di stabilire una connessione client crittografato a una rete virtuale in Azure. Questo approccio è ideale per i computer client e le connessioni da sito a sito di dimensioni ridotte.